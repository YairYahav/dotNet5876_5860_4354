...
public static class Factory
{
    public static IDal Get
    {
        get
        {
            string dalType = DalConfig.s_dalName ??
                throw new DalConfigException($"DAL name is not extracted from the configuration");

            DalConfig.DalImplementation dal = DalConfig.s_dalPackages[dalType] ??
            ...
            return type.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static)?.GetValue(null) as IDal ??
                throw new DalConfigException($"Class {dal.Class} is not a singleton or wrong property name for Instance, or Instance is not of type IDal.");
        }
    }
}